...
export class AuthService {
  ...
  async signUp(signUpDto: SignUpDto, fingerprint?: any): Promise<SignUpResponseDto> {
    ...
    try {
      ...
      try {
        ...
        // In development mode, continue even if email fails (user can get code from logs/response)
        if (process.env.NODE_ENV === 'development') {
          this.logger.warn(`⚠️ Development mode: Email failed but allowing signup to continue`);
          this.logger.warn(`⚠️ Verification code for ${email}: ${verificationCode}`);
        }
      }
      
      // In production, if email fails, cleanup and throw error
      if (!emailSent && process.env.NODE_ENV !== 'development') {
         ...
         const errorMessage = emailError instanceof Error ? emailError.message : 'Email service is unavailable';
         this.logger.error(`❌ Production mode: Blocking signup due to email failure: ${errorMessage}`);
         throw new BadRequestException(`Cannot send verification email at this time. Please try again later or contact support.`);
      }
  ...
  async resendVerificationCode(email: string): Promise<{ message: string; previewUrl?: string; code?: string }> {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        this.logger.warn(`⚠️ Development: Email sending failed`);
      }
      ...
      return {
        ...
        previewUrl: process.env.NODE_ENV === 'development' ? `Code: hidden` : undefined,
        code: process.env.NODE_ENV === 'development' ? 'DEBUG_ONLY' : undefined,
      };
    }
  ...
  async login(loginDto: LoginDto, ipAddress?: string, userAgent?: string, fingerprint?: any, subdomain?: string | null, tenantDomain?: string | null): Promise<LoginResponseDto> {
    ...
    if (!resolvedSubdomain && tenantDomain) {
      ...
      } else if (tenantDomain.endsWith('.kawn.com') || tenantDomain.endsWith('.kawn.net')) {
    ...
    */
    ...
    if (user.tenantId) {
      ...
      // CRITICAL FIX: Auto-create default tenant for SHOP_OWNER users without one
      // This prevents foreign key constraint violations when creating brands, products, etc.
      this.logger.warn(`🔧 SHOP_OWNER user ${user.id} (${user.email}) has no tenant. Creating default tenant...`);
      
      try {
        // Generate a unique tenant ID and subdomain
        const tenantId = crypto.randomBytes(50).toString('hex');
        const subdomain = `store-${user.email.split('@')[0]}-${crypto.randomInt(1000, 9999)}`.toLowerCase();
        ...
        this.logger.log(`✅ Created default tenant ${tenant.id} (subdomain: ${tenant.subdomain}) for user ${user.id}`);
        
        // Link the user to the new tenant
        await this.linkUserToTenant(user.id, tenant.id, true);
        this.logger.log(`✅ Linked user ${user.id} to default tenant ${tenant.id}`);
        
        // Update the user's active tenantId
        await this.prismaService.user.update({
          where: { id: user.id },
          data: { tenantId: tenant.id },
        });
        
        // Update the user object so it's used in token generation
        user.tenantId = tenant.id;
        user.tenant = tenant;
        
        this.logger.log(`✅ Set default tenant ${tenant.id} as active for user ${user.id}`);
      } catch (error) {
        this.logger.error(`❌ Failed to create default tenant for user ${user.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        // Don't fail login - user can create tenant manually later
      }
    }
