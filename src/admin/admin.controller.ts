import { Controller, Post, Delete, Put, Get, Body, HttpCode, HttpStatus, Query, Param, Logger, UseGuards } from '@nestjs/common';
import { RateLimitingService } from '../rate-limiting/rate-limiting.service';
import { PrismaService } from '../prisma/prisma.service';
import { CloudinaryAccessService } from './cloudinary-access.service';
...
export class AdminController {
  ...
  constructor(
    ...
    private cloudinaryAccessService: CloudinaryAccessService,
  ) {}
  ...
  async getRateLimitStats() {
    ...
    return {
      total,
      byType: stats.map((s: any) => ({ type: s.type, count: s._count.id })),
    };
  }
  ...
  async getCloudinaryAccessUsers() {
    return this.cloudinaryAccessService.getCloudinaryAccessUsers();
  }

  @Post('cloudinary-access')
  @UseGuards(JwtAuthGuard)
  @HttpCode(HttpStatus.OK)
  async updateCloudinaryAccess(
    @Body() data: { userIds: string[]; hasAccess: boolean; grantedBy?: string },
  ) {
    const grantedBy = data.grantedBy || 'system-admin';
    return this.cloudinaryAccessService.updateCloudinaryAccess(
      data.userIds,
      data.hasAccess,
      grantedBy,
    );
  ...
  @Get('cloudinary-access/:userId')
  @UseGuards(JwtAuthGuard)
  async getUserCloudinaryAccess(@Param('userId') userId: string) {
    return this.cloudinaryAccessService.getUserCloudinaryAccess(userId);
  }
}
